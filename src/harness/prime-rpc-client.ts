 ...
 */
import { createChildProcessIo, type PrimeRpcIo } from "./prime-rpc-io.ts";
...
export interface PrimeRpcClientOptions {
  ...
  /** Override the process I/O (e.g. run inside a QM sandbox). Default: child process. */
  io?: PrimeRpcIo;
...
export class PrimeRpcClient {
  private io: PrimeRpcIo | null = null;
  private buffer = "";
  ...
  async start(): Promise<void> {
    if (this.io) throw new Error("client already started");
    const args = ["--mode", "rpc"];
    ...
    this.io = io;
    io.onData((chunk) => this.handleChunk(chunk));
    io.onExit((code) => {
      this.exitCode = code;
    });
    await io.start();
    if (this.exitCode !== null) {
      throw new Error(`prime-agent exited immediately (code ${this.exitCode}): ${io.stderrTail().slice(-1500)}`);
    }
  }

  /** LF-only JSONL framing from raw chunks. */
  ...
  async send(command: Omit<RpcCommand, "id">, opts: PrimeRpcSendOptions = {}): Promise<RpcResponse> {
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error(`timeout waiting for ${command.type} (${timeoutMs}ms). stderr: ${io.stderrTail().slice(-500)}`));
      }, timeoutMs);
