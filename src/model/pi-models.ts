...
export { MODEL_REGISTRY } from "./model-registry.ts";
...
export const HARNESS_IDS = ["pi", "opencode", "codex", "claude", "mock", "prime", "hermes"] as const;
...
export function resolveCoreModel(id: string): PiModel | undefined {
...
export function resolveModel(id: string): PiModel | undefined {
  return resolveCoreModel(id) ?? (resolveCustomModel(id) as unknown as PiModel | undefined);
}
...
export function modelSupportedByHarness(id: string | undefined, harness: string): boolean {
  ...
  if (harness === "prime" || harness === "hermes") return provider === "deepseek" || provider === "openai" || Boolean(resolveModel(id));
  return false;
}
