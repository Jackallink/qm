...
- `SESSION_STORE=postgres` and `RUN_STORE=postgres` are both mandatory for Gate 2 (memory run store would make admission rows, claimRemoteOnce, and terminal write-through process-local and non-durable); shared pool per connection string for the admission transaction domain.
...
- `sharedPgPool` accumulates DDL per connection string: the registry keeps a per-connection-string set of applied statement texts and applies each new caller's unseen statements on that caller's first use (semantics (a) — no reliance on `IF NOT EXISTS` idempotency and no claim that `applyDdl` itself deduplicates; `applyDdl` in `pg-pool.ts` executes statements in order, the registry owns the seen-set).
...
In `src/persistence/pg-pool.ts`, add a module-level `Map<string, { pool: PgPool; refs: number; applied: Set<string> }>`; `sharedPgPool` returns the existing entry or creates one via the existing `createPgPool`; on first use by each caller, statements not present in `applied` are run through `applyDdl` and added to `applied` (semantics (a): registry owns the seen-set, `applyDdl` stays order-preserving). `close()` on the shared wrapper decrements refs and ends the pool at zero.
...
- DDL: `remote_runtime_binding(id TEXT PK, version INT, enabled BOOL, configured_org_id TEXT, allowed_scope_id TEXT, protocol_version INT, runtime_audience TEXT, transport_service_id TEXT, transport_certificate_pin TEXT, release_digest TEXT, release_attestation_key_id TEXT, receipt_key_set_version INT, metering_key_set_version INT, max_input_bytes INT, max_history_messages INT, max_output_bytes INT, max_runtime_ms INT, token_ttl_ms INT, budget_ceiling_usd NUMERIC, key_sets JSONB, policy_snapshot_hash TEXT, created_by TEXT, created_at BIGINT, disabled_by TEXT, disabled_at BIGINT)`; `key_sets` holds FOUR ordered key sets: the core turn/abort **verification public key set**, the **attestor public key set** (for pre-claim attestation / start proof / termination proof header-kid verification — `release_attestation_key_id` names the current entry, the set enables rotation), the receipt key set, and the metering key set, each `[{kid, publicKeyPem, state: current|overlap, activatedAt, retiresAt}]`. `remote_turn_events(id BIGSERIAL, remote_turn_id TEXT, seq INT, event_type TEXT, payload JSONB, created_at BIGINT, UNIQUE(remote_turn_id, seq))`.
...
- Events include: `session_bind`, `admit`, `prepare_dispatch`, `abort_pre_claim`, `claim`, `start`, `attestation_invalid` (post-claim start-proof mismatch → `runtime_attestation_invalid` park), `receipt`, `teardown`, `complete`, `abort`, `timeout`, `park`, `reconcile_completed`, `reconcile_cancelled`, `reconcile_failed`, `reject_deadline`, `expire_pre_dispatch`, `duplicate_receipt`.
...
**Interfaces:**
- Consumes: `sharedPgPool`, `withPgTransaction`, `createRemoteBindingStore`, session store's transaction-bound `getOrCreateByThreadOn(client, ...)` (add this variant to `src/sessions/postgres-session-store.ts` — it reuses ONLY the SQL shape, NOT the heal branch, per spec "no heal inside admission"; heal stays on the pool-level public path), `state-machine.ts`, `reserveBudget` from Task 6, `computeInputDigest`/`computeHistoryDigest`/`computeEnvelopeDigest` from `src/remote-turn/envelope.ts` (Task 7 — **Task 5 depends on Task 7's envelope module**; digests are computed exactly once at admission and persisted, never recomputed at dispatch).
- Produces: `admit(input: AdmitInput): Promise<AdmitResult>` — **creates the `remote_once` run row + mints its lease BEFORE the admission transaction** (enqueue-run-first, spec), then in one transaction: locks enabled binding, validates G0 context (interface placeholder `G0Context`), bounds, resolves/creates session (ON CONFLICT DO NOTHING + FOR UPDATE re-select), **acquires the `session_leases` row with holder `remote_turn:<id>`** (rejecting a second active remote turn on the same session with `remote_refused: remote_turn_active`), computes and persists `input_digest`/`history_digest`/`envelope_digest` (history digest column added to the `remote_turn` DDL in Task 3), INSERTs `remote_turn(created→session_bound→admitted)` with version CAS, writes `remote_turn_events`, reserves budget (Task 6), returns `{ status: "admitted", remoteTurnId, coreRunId, runLeaseToken }` with no JTI. Denials return `{ status: "refused", reason: "governance_authorization_required" | ... }` with durable denial event, released session lease, and no reservation. On refusal, the pre-created `remote_once` run row is marked `failed` (orphan policy, recorded in Task 13). `runLeaseToken` is the single authoritative run lease: `claimRemoteOnce` (Task 9) and the terminal CAS write-through (Tasks 10/11) use the SAME token — there is exactly one `runs.lease_token` per row.
...
**Interfaces:**
- Produces: `RemoteTurnKeyProvider { getCurrentSigningKey(): { kid: string; privateKeyPem: string } }` — the signer injection point; Gate 2 wires a fixture provider in tests and `createRemoteTurnStore` options; the deployment-layer env (`REMOTE_TURN_SIGNING_KEY`) assembly is recorded in Task 12 as Gate 3 wiring scope. `mintTurnToken(payload: TurnClaims, key: { kid: string; privateKeyPem: string }): Promise<string>` and `verifyTurnToken(token, keys: CoreTokenKeySet, expected: { aud: string; envelopeDigest: string; remoteTurnId: string; now: number }): Promise<TurnClaims | null>` (jose CompactSign/compactVerify, Ed25519 with `algorithms: ["EdDSA"]` closed set, kid from the binding's pinned set — **header kid must equal payload kid and be in the set**; `expected.aud` exact-match with `binding.runtimeAudience`, `expected.envelopeDigest` compared against the persisted value); `mintAbortToken(abortClaims, key)`, `verifyAbortToken(token, keys, expected, turnState)` where `turnState: { phase: "pre_claim" | "post_claim"; persistedExecutionLeaseHash?: string }` (pre-claim form binds turnJtiHash only; post-claim form additionally requires `executionLeaseHash` === persisted hash and `coreRunId`); `csprngHex(bytes = 16): string` via `randomBytes` (128-bit, `randomUUID`'s 122 bits is insufficient); `envelope.ts` exports `computeInputDigest(text)`, `computeHistoryDigest(history)`, `computeEnvelopeDigest(fields)` per the length-prefix framing in Global Constraints; `claim(tx, { remoteTurnId, turnJtiHash, attestationNonceHash, verifiedPreClaim, now }): Promise<LeaseResult>` — **takes the PRE-VERIFIED attestation result** (verification ran outside the transaction, per Task 8; the raw JWS never enters the transaction), then atomic JTI consumption with the CAS guard `UPDATE remote_turn SET status='claimed', execution_lease_hash=$2, version=version+1 WHERE id=$1 AND status='dispatching' AND turn_jti_hash=$3 AND abort_requested_at IS NULL AND version=$4`; zero rows → `{ ok: false, reason: "no_lease" }`; on success mints the lease-bound abort authority (post-claim abort token with `executionLeaseHash` + `coreRunId`).
...
**Interfaces:**
- Produces: `verifyPreClaimAttestation(jws, { attestationKeySet, expected: { remoteTurnId, bindingVersion, turnJtiHash, attestationNonceHash, intendedWorkloadIdentity, releaseDigest, policyDigest, endpointAllowlist, egressAudience, expiry, singleUse } }): Promise<PreClaimClaims | null>` and `verifyStartProof(jws, { attestationKeySet, expected: { executionLeaseHash, plannedSandboxId, intendedWorkloadIdentity, turnJtiHash } }): Promise<StartProofClaims | null>` — jose Ed25519 real signatures per 05-protocol-schema.md; `attestationKeySet` comes from the binding's `key_sets.attestor` (Task 3). Task 8 ships ONLY the pure verify units + their unit tests; the claim-integration assertions (reuse, wrong-lease, mismatch → park) run after Task 7's `claim()` exists (see Task 7 Step 1 additions), so Task 8 does not depend on Task 7 but Task 7 depends on Task 8's verifiers.
- The pre-claim attestation contains NO execution lease (05), so "attestation claiming an unissued/wrong lease" is a **start-proof** negative, not a pre-claim one — placed in Task 10's start-proof consumption tests.
...
**Files:**
- Modify: `src/remote-turn/store.ts`, `src/runs/run-store.ts` (add `claimRemoteOnce` to the `RunStore` interface), `src/runs/postgres-run-store.ts` + `src/runs/memory-run-store.ts` (implement `claimRemoteOnce` — `lease_token + status` CAS, pending → running, NO session guard — the remote row is the session's only active run by construction)
- Test: `test/remote-turn-recovery-pg.test.ts` (pg)

**Interfaces:**
- Produces: `prepareDispatch(tx, { remoteTurnId, leaseToken, envelope, now }): Promise<DispatchResult>` — partial unique index on `admission_key` over non-terminal states; writes turn JTI hash, attestation nonce hash, dispatch owner/attempt, `preClaimExpiresAt` (DB clock) before any network call; only the lease-owner (runs.lease_token + status CAS) may resume pre-claim states — **`leaseToken` here is the `runLeaseToken` returned by `admit` (Task 5); there is exactly one run lease per remote row**; the first write guards `WHERE status='admitted' AND turn_jti_hash IS NULL AND version=$v` and a resume SELECT guards `status='dispatching' AND transaction_timestamp() <= pre_claim_expires_at` — a resume NEVER updates the JTI/nonce columns (no regeneration); `expirePreClaim(remoteTurnId, now)` → `failed_pre_dispatch` with JTI revoked, reservation/lease released (the run row is written `failed` via `runs.fail` in the same transaction); restart resends only the persisted envelope (no new JTI/nonce) and **never recomputes the envelope digest — `envelope_digest`/`history_digest`/`input_digest` were persisted once at admit and are only read here**. The dispatch coordinator moves the run `pending → running` with `claimRemoteOnce(runId, runLeaseToken, workerId, ttlMs)` (Task 9's new RunStore method) using the same token.
...
**Interfaces:**
- Consumes: `settleReservation` (Task 6), `verifyStartProof` (Task 8), `runs.complete`/`runs.fail` (the existing RunStore CAS methods — the runs terminal write-through MUST go through them, NOT raw SQL, so `settle()` fires the `onTerminal` listeners: run-result delivery (the final reply's only outlet), `engaged.settle`, orphan-signal replay, session-state terminal emit).
- Produces: `verifyReceipt(token, receiptKeySetSnapshot, expected: {...}): Promise<ReceiptClaims | null>` (schema per 05, `status: "completed"` const, key-set snapshot at admission, `current ∪ overlap`); store transitions `executing → reply_received` (valid receipt), `reply_received → teardown_pending` (trusted usage), `teardown_pending → completed` (attestor deletion + egress revocation proof — Gate 3 provides real proofs, here the store accepts a `TerminationEvidence` interface double); `reply_received → parked` (invalid metering); **`claimed → executing` consumes `verifyStartProof`** (planned-vs-actual mismatch → `runtime_attestation_invalid` park, using Task 4's `attestation_invalid` event and the attestor key set from Task 3); **every terminal transition writes the run row in the SAME transaction via `runs.complete(runId, runLeaseToken, result)` / `runs.fail(...)`** using the `runLeaseToken` returned by `admit` (Task 5) — `completed` → `done`, `cancelled`/`failed`/`failed_pre_dispatch`/`rejected` → `failed` — so the `onTerminal` chain fires exactly once and the final reply is delivered.

- [ ] **Step 1: Write failing tests** — valid receipt moves executing→reply_received; wrong/retired key, bad signature, wrong lease/input/release digest, schema violation (unknown field, >16 KiB reply byte-bound), duplicate completed receipt (ignored + audit), late post-cancel receipt (ignored), missing trusted usage → full charge; invalid metering → parked; teardown with deletion+revocation proof → completed; without proof → parked; **start-proof planned-vs-actual mismatch → `runtime_attestation_invalid` park; attestation claiming an unissued/wrong lease denied (start-proof negative, 05: pre-claim attestation contains no lease)**; **terminal transitions assert the run row flipped `done`/`failed` via the CAS with the admit-returned `runLeaseToken` AND that the `onTerminal` listeners fired (e.g. run-result delivery observed)**.
...
- [ ] **Step 1: Write the validation record** — table of RTH → test file → status; record the pg suite + typecheck outputs verbatim. The drift/partial section MUST list explicitly: (a) RTH-04 envelope negatives (>32 KiB, attachment/tool/path/credential/binary) partially covered by Task 5 bound validation only; (b) RTH-11's 60s runtime ceiling partially covered — sweeper-side transition in Task 11, no fake-runtime execution side (Gate 3); (c) RTH-06's matrix file `remote-turn-session-binding-pg.test.ts` and RTH-11's `remote-turn-cancellation.test.ts` were folded into `remote-turn-store-pg.test.ts` (Task 5) and `remote-turn-reconciler-pg.test.ts` (Task 11) respectively; (d) RTH-08's unregistered-service and DNS/cert/SAN transport negatives have no Gate 2 task (no transport resolver — Gate 3), recorded as partial; (e) the real turn-flow admission entry point (app-turn branch / G0-presence routing) is Gate 3 — Gate 2's `admit()` is exercised by pg tests only; (f) `REMOTE_TURN_SIGNING_KEY` provider assembly is Gate 3 wiring — Gate 2 uses the injected fixture provider; (g) RTH-02/03 are covered by Task 3 (binding enable/disable race) and Task 5 (G0 denial) — map them in the table rather than excluding them as "not Gate 2".
...
- [ ] **Spec coverage:** RTH-05 (Task 5), RTH-06 (Task 5), RTH-07 (Task 7), RTH-08 (Task 8 pure verify + Task 7/10 integration; layer egress in Gate 3), RTH-09 (Task 9 + Task 11), RTH-10 (Task 10), RTH-11 (Task 11), RTH-12 (Task 12), RTH-13 (Task 12). RTH-02 (Task 3) and RTH-03 (Task 5) are mapped despite the "1–2" matrix gate column; RTH-01/RTH-04/RTH-14 are Gate 0/partial/layer-scope — RTH-04's envelope negatives and RTH-11's runtime ceiling are partial coverage, listed explicitly in Task 13.
...
- [ ] **Parallelism:** Tasks 1, 2, 3, 4, 6 are wave 1 (independent); 5 depends on 1/2/3/4/6 AND on Task 7's `envelope.ts` digest module (digests computed once at admit); **7 depends on 8** (claim takes the pre-verified attestation result) and 8 depends on 3/4 (pure verify units, no claim dependency — its claim-integration assertions move to Task 7's test step); 9 depends on 5/7; 10 depends on 5/6/7/8/9; 11 depends on 9; 12 depends on 10; 13 last. Parallel writing is fine; pg test suites must run serially (`--test-concurrency=1`) due to shared `DATABASE_URL` DROP coordination.
