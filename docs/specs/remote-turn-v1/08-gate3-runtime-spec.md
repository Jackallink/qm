...
- `POST /attest/pre-claim` — input: the core-issued attestation nonce plus
  turn-token facts relayed from the runtime's claim flow (the nonce is
  generated by core at `prepareDispatch`; the runtime relays it). **Creates
  ...
  no execution lease.
- `POST /lease` — core delivers the execution lease here (never via runtime).
  On receipt: mints the scoped egress token with the egress gateway (the
  token binds `executionLeaseHash`, which only exists now), injects it into
  the stopped sandbox's env, **starts** the container, verifies running
  state, signs and returns `start_proof` (`sandboxId` must equal the
  pre-claim `plannedSandboxId`; core enforces the equality — already
  implemented in `attestation.ts`).
...
- `POST /v1/remote-turn/claim` (runtime→core): { turn token, attestation
  ...
  `store.claim(...)`. On success core pushes the execution lease to the
  attestor's `POST /lease` (§3.2) itself — the lease never transits the
  runtime.
- `POST /v1/remote-turn/start-proof` (attestor→core): start proof JWS →
  `store.startExecution(...)`.
- `POST /v1/remote-turn/receipt` (runtime→core): receipt JWS →
  `store.receiveReceipt(...)` → `beginTeardown(...)`; core then calls the
  attestor's `POST /terminate`.
- `POST /v1/remote-turn/usage` (egress-gw→core): usage statement JWS →
  **new verifier** (05 usage schema, `kid` ∈ binding `meteringKeys`, lease
  hash binding) → consumed by `completeTeardown`'s settlement as
  `trustedUsageUsd`. Absent/late statement ⇒ `trustedUsageUsd: null`
  (full charge, invariant 9).
...
| mTLS certs ×4 links | each service | transport identity | deployment procedure, cert expiry monitored |
...
| G3-16 | claim endpoint (§5.4a): valid pre-claim JWS → lease; forged/expired/wrong-kid JWS → typed refusal; lease delivered to attestor, never to runtime | core integration |
| G3-17 | usage verifier: valid statement settles partial; wrong kid/schema violation/no statement ⇒ full charge | core integration |
