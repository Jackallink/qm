...
| Created/session-bound/admitted pre-dispatch deadline expires | Dispatch coordinator | `remote_refused: pre_dispatch_expired` | Admission key, deadline, release of session lease/reservation, no remote request/JTI | RTH-05, RTH-09 |
...
| Attestation/digest/JTI/nonce/workload/transport/network proof invalid or lease-order confused | Attestor verifier | `remote_refused: runtime_attestation_invalid` | Attestation/transport digest and reason, no lease | RTH-08 |
...
| Invalid receipt or invalid trusted usage while execution is active | Receipt settler | `remote_parked: receipt_unverified` | Receipt/metering digest, key ID, schema result, reservation state | RTH-10 |
...
| One-time authority | Exactly one core claim returns execution lease and a distinct abort capability reaches the runtime only from core. | Replayed or cross-purpose turn/abort token, wrong request digest, bad `kid`, skewed clock, duplicate claim, unauthorized user abort, client disconnect, and concurrent abort produce no extra lease/capability. |
| Trusted execution | Binding-pinned attestation and static mTLS transport verify exact release/image/turn/JTI/nonce/workload/sandbox/network policy before lease. | Runtime self-report, attestation replay, nonce/workload/transport mismatch, DNS/cert mismatch, host mode, writable mount, direct network, metadata/internal IP, proxy bypass, egress-token copy, or non-model endpoint is denied. |
